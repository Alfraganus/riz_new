...
class OpenAiTextChatService extends OpenAiService
{

   public function sendTextChatToGPT(Request $request) {
        ...
        foreach ($request->input('messages') as $message) {
            $messagePayload[] = [
                'role' =>'user',
                'content' =>sprintf("%s said: %s", $message['role'],  $message['text']),
            ];
        }
                ...
                'json' => [
                    ...
                    'messages' => array_merge(
                        [
                            [
                                'role' => 'system',
                                'content' => self::getGPTCommands($request->input('response_level'), 'text_chat'),
                            ],
                        ],
                        $messagePayload
                    ),
